...
- 在完成后端单元测试和前端组件 / 状态单元测试后，必须增加真实前端调用真实后端的用户场景测试。
- 集成测试应启动真实 FastAPI 应用和 `ui/V1/` 前端构建或开发服务器，最终验收必须优先通过 Codex 内部浏览器模拟用户点击、输入、提交、等待和刷新/跳转确认结果，不使用 mock API 替代后端。
- 场景应覆盖：进入全局项目、打开任务列表、创建任务、选择可用工作流、填写文本 / 图片 / 文档输入、运行任务、接收节点级 SSE 更新、在候选图片中选择 1 个、提交交互继续执行、查看最终输出。
- 场景应覆盖节点重跑：对已完成生图节点执行重新运行，确认下游当前结果被清空，等待新候选图输出，选择新结果后继续下游执行。
- 场景应覆盖历史任务：打开已完成任务时使用任务自己的 workflow snapshot，不受作者后续修改工作流 YAML/JSON 影响。
- 集成测试可以使用测试专用工作流和 mock 节点输出图片数据，但调用路径必须经过真实 API、真实任务运行时、真实 SSE 和真实前端页面。
...
- 测试策略覆盖后端单元测试、前端单元测试和真实前后端联调的用户场景测试。
