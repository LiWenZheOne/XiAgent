...
- `asset_tags` 是类型内部标签，只表达稳定造型、配件、地点属性、道具分类等信息。标签中不得包含 `角色`、`地点`、`道具` 这类一级类型，也不得重复 `asset_name`。业务匹配时把 `asset_tags` 当作无序集合处理；标签顺序只影响展示，不影响匹配、重复检查或选择。
...
资产库记录的 `name` 只允许作为可读标题、下载文件名、错误提示和列表展示文案。工作流新契约、资产匹配、重复检查和资产选择不得依赖复合 `name`，也不得通过拆分 `name` 来决定资产身份。需要判断已有资产是否可用时，必须使用资产库标签中的一级类型标签、主名称标签和类型内部标签。
...
资产选择框、资产匹配候选列表和入库重复检查按结构化条件筛选：

1. 先按 `asset_type` 过滤，例如角色只看 `character`。
2. 再按 `asset_name` 精确匹配业务身份，例如角色只看 `何涛`。
3. 再按 `asset_tags` 做集合匹配，例如目标是 `["官兵装束", "佩刀"]`，已有资产只要包含这些标签即可作为可用匹配；标签顺序不得参与判断。
4. UI 可以继续显示复合标题，例如 `角色_何涛_官兵装束_官帽、佩刀`，但这个标题只用于阅读，不作为匹配键。
