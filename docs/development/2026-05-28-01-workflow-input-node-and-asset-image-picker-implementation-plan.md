...
**Architecture:** 后端新增系统输入节点与运行时恢复语义，用户提交该节点后把输出固化为 `$workflow.input`。前端创建任务页只展示工作流说明，任务详情用同一套 `node-ui` 控件渲染起始输入节点；资产图片选择作为控件库能力复用在 workflow 输入表单中。
...
- `xiagent/ui_controls/catalog.py`：注册 `ui.input.workflow_form.v1` 和 `ui.input.asset_image_picker.v1`。
...
- `ui/V2/src/node-ui/controls/WorkflowInputFormControl.tsx`：新增起始输入表单控件。
...
def _should_defer_workflow_input(contract: dict[str, Any], input_data: dict[str, Any]) -> bool:
    return input_data == {} and bool(_workflow_input_node_ids(contract))
```
...
```python
defer_workflow_input = _should_defer_workflow_input(contract, input_data)
if not defer_workflow_input:
    validate_json_value(contract["workflow"]["input_schema"], input_data)
```

保持 `input_json` 写入 `{}`，然后调用 `_continue_task(... workflow_input=input_data ...)`。
...
def test_workflow_input_controls_are_registered() -> None:
    ...
    assert "ui.input.workflow_form.v1" in control_ids
...
UiControlDescriptor(
    ...
    kind="input",
    tags=("workflow", "input", "form", "interactive"),
    variants=(
        UiControlVariant(
            name="default",
            label="工作流输入表单",
    ...
    description="在任务详情首个输入节点中收集 workflow input。",
),
UiControlDescriptor(
    control_id="ui.input.asset_image_picker.v1",
    version="1.0.0",
...
如果 `ui.input.asset_image_picker.v1` 只作为 `workflow_form` 的字段子控件使用，不直接作为 slot 控件，则不需要字段级 manifest 校验；字段级配置由前端控件内部处理，后端只校验外层 `ui.input.workflow_form.v1`。
...
3. 对 `runninghub_image_to_image_test.workflow.yaml` 的 `image_urls` 字段在 `workflow_form` 下配置资产图片控件：
...
- Create: `ui/V2/src/node-ui/controls/WorkflowInputFormControl.tsx`
...
```tsx
it("renders workflow input form and submits image urls as an array", async () => {
  const onSubmit = vi.fn();
  render(
    <WorkflowInputFormControl
      config={{
        control_id: "ui.input.workflow_form.v1",
        variant: "default",
        mode: "input",
...
export interface WorkflowInputFieldControlConfig {
  control_id: string;
  variant?: string;
  mode?: string;
...
export interface WorkflowInputFormOptions {
  fields?: Record<string, WorkflowInputFieldControlConfig>;
}
```

- [ ] **Step 3: 实现 WorkflowInputFormControl**

创建 `ui/V2/src/node-ui/controls/WorkflowInputFormControl.tsx`，核心结构：

```tsx
import { useMemo, useState } from "react";

import type { JsonSchema } from "../../api/types";
import type { NodeUiControlProps, WorkflowInputFormOptions } from "../types";
...
export function WorkflowInputFormControl({ config, node, busy = false, onSubmit }: NodeUiControlProps) {
  const schema = schemaFromNode(node);
  const options = (config.options ?? {}) as WorkflowInputFormOptions;
...
在 `WorkflowInputFormControl.tsx` 中为非图片数组字段加入固定分支：
...
```ts
import { WorkflowInputFormControl } from "./controls/WorkflowInputFormControl";
```

并在 `nodeUiRegistry` 增加：

```ts
"ui.input.workflow_form.v1": WorkflowInputFormControl,
```

`ui.input.asset_image_picker.v1` 作为 `WorkflowInputFormControl` 的字段控件，不直接进入 `nodeUiRegistry`，避免 slot 控件和字段控件 props 混用。
...
在 `ControlLibraryPage.tsx` 中对 `ui.input.workflow_form.v1` 增加预览。
...
```powershell
git add ui/V2/src/node-ui/controls/AssetImagePickerControl.tsx ui/V2/src/node-ui/controls/WorkflowInputFormControl.tsx ui/V2/src/node-ui/fixtures/workflowInput.ts ui/V2/src/node-ui/registry.ts ui/V2/src/node-ui/types.ts ui/V2/src/node-ui/ControlLibraryPage.tsx ui/V2/src/styles/app.css ui/V2/src/tests/node-ui.test.tsx
...
```tsx
it("creates a task from launch information without rendering workflow input form", async () => {
  render(<App />);
...
- `ui.input.workflow_form.v1` 负责渲染起始输入节点，字段级资产图片选择使用 `ui.input.asset_image_picker.v1`。
```
...
- 类型一致性：后端统一使用 `system.workflow_input.v1`；前端 slot 控件使用 `ui.input.workflow_form.v1`，字段级图片控件使用 `ui.input.asset_image_picker.v1`。
- 兼容策略：保留非空 `input_data` 旧调用路径；V2 新路径省略 `input_data`；显式迁移全局工作流。
