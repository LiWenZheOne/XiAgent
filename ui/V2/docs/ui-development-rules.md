...
- 可复用节点 UI 已建立在 `ui/V2/src/node-ui/`：
  ...
  - `ui.interaction.asset_summary_table.v1` 用于 P3 资产列表汇总节点，按角色、地点、道具三个 tab 展示资产列表；一行一个资产，列出该资产已有字段、单一 `匹配资产` 字段和图像列。匹配资产字段可打开按类型过滤且可搜索的资产选择框；列表必须支持新增和删除行，提交 `approved_assets` 时以后端收到的编辑后列表为准；图像列支持逐行选取本地图像上传，提交 `decision`、`approved_assets` 与已补充的 `asset_images`。
  - `ui.interaction.asset_image_cards.v1` 用于工作流等待节点中的资产补图工作台，按角色、地点、道具分组展示资产卡片；卡片展示匹配结果、变体、配件、提示词和参考图，只提供逐卡上传/选取图片入口，不展示公开图片 URL 输入；提交 `decision` 与已上传的 `asset_images`，后端再只生成未上传资产。
  - 每个控件有稳定 `control_id`。
...
- 所有工作流都应优先通过 `workflow.ui.stages` 定义 P 阶段视图；P1/P2 这类顶层单元称为阶段卡片，阶段内部采用 S1/S2 步骤记录流而不是嵌套卡片。任何配置了 stages 的工作流，只有当前页面发起“运行下一步”或“重新运行”后的新增节点才逐步揭示 S 步骤；进入运行态前必须把当前已知事件记为基线，之后只由新推送的节点事件推进步骤显示。切换任务、切换工作流、刷新页面或回看历史任务时，已有真实执行记录必须直接显示，不能播放旧步骤。运行期间不能因为任务详情快照已经包含多个 `node_execution_id` 就一次性预展示本次新产生的阶段节点，也不能出现后续步骤先完成、前序步骤还在运行的视觉错序；已完成历史任务可以直接显示全部真实执行记录。P 阶段必须可收起，运行时自动展开包含当前运行、等待、失败或错误步骤的 P；P 标题栏不显示展开/收起按钮，点击标题栏本身展开，再次点击收起，并支持键盘 Enter/Space。P 阶段收起时显示横向节点进度条，按实际出现的 S 步骤展示圆点、标题和状态；P 阶段展开后隐藏该进度条，避免和内部步骤列表重复。S 步骤编号在整个工作流内跨 P 连续递增，不在每个 P 内重新从 S1 开始。每条步骤记录直接显示进度条、百分比、状态和当前动作；运行中的记录不显示独立转圈图标，只在进度条上使用流动动画，等待中的输入节点不显示运行动画。S 步骤详情按是否需要用户处理分类：等待用户输入、确认或选择的步骤自动展开详情；纯运行中、已完成或无需用户动作的步骤默认只显示记录行，用户点击后才展开详情。步骤操作必须跟随具体 S 记录显示：当前等待输入步骤显示 `运行下一步`，已完成步骤固定显示 `重新运行`，未到达步骤不显示操作；P 阶段标题栏不显示 `运行下一步` 或 `重新运行`，只保留阶段标题、说明和状态。重新运行前必须提示用户该步骤及下游结果会失效并从此处重跑，历史记录保留；确认后必须清空该步骤及其下游 S 显示状态，再由新事件重新生成。点击记录后只在该记录下展开节点详情内容，不再出现第二层节点标题卡片。未配置 stages 的旧工作流仅作为兼容路径回退到一节点一卡片视图，新建或重构工作流不得依赖旧回退视图。阶段设计细节见 `2026-05-29-01-workflow-stage-view-design.md`。
...
- 图片展示类控件和资产补图卡片使用瀑布流列布局，保留图片自然比例和卡片真实高度；不得为了行对齐强制所有缩略图或补图卡片等高。
